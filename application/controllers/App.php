...
class App extends CI_Controller
{
	...
	public function index()
	{
		...
		// $sekolah = $this->Master_model->read_data();
	    // $get = $this->db->query("SELECT * FROM mst_sekolah WHERE id = 1")->row();
		// $d['nama_sekolah'] = $sekolah->pr_nama;
        // $d['logos'] = $sekolah->pr_logo;
        // $d['alamat'] = $sekolah->pr_alamat;
		// $d['title'] = "Home";
...
//		$get = $this->db->query("SELECT mst_siswa.id_kelas,nama_kelas,nama_siswa FROM mst_siswa INNER JOIN mst_kelas ON mst_siswa.id_kelas = mst_kelas.id_kelas WHERE nis = '$nis'");
		$get = $this->db->query("SELECT sr_siswa.idsiswa as id_kelas,sr_kelas.k_keterangan as nama_kelas,sr_siswa.s_nama as nama_siswa FROM sr_siswa INNER JOIN sr_kelas ON sr_siswa.idkelas = sr_kelas.idkelas WHERE s_nisn = '$nis'");
		$arr = array();
		if($get->num_rows() > 0) {
	...
	{
		$get = $this->db->query("SELECT * FROM sr_profile WHERE idprofile = 1")->row();
		$d['pr_nama'] = $get->pr_nama;
		$d['title'] = "Home";
		$d['buku'] = $this->Master_model->buku();
		$d['pengunjung'] = $this->Master_model->pengunjung_today();
		$this->load->view('front/top', $d);
	...
	public function save_pengunjung()
	{
		...
		$this->session->set_flashdata('success', 'Data Ter simpan');
		redirect('app/home');
	}
