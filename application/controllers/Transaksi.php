...
class Transaksi extends CI_Controller
{
	...
	public function ajax_combo_buku()
	{
		...
		$get = $this->db->query("SELECT * FROM mst_buku WHERE id_buku = '$id_buku'");
		$arr = array();
		foreach ($get->result_array() as $data) {
			$stok = $this->db->query("SELECT COALESCE(SUM(jumlah),0) as hitung FROM peminjaman_buku_dt WHERE id_buku = '$data[id_buku]' AND status_pinjam_dt = '1'")->row();
	...
	public function proses_cari_pinjam()
	{
		...
		redirect("transaksi/peminjaman/" . $this->input->post("s_nisn"));

	}

	public function listpeminjaman()
	{
		$d['judul'] = "List Peminjaman Buku";
		$d['peminjaman'] = $this->Transaksi_model->peminjaman_tigabulan();
		$this->load->view('top', $d);
		$this->load->view('menu');
		$this->load->view('transaksi/daftar_peminjaman_all');
	...
	public function listpengembalian()
	{
		...
		$this->load->view('top', $d);
		$this->load->view('menu');
		$this->load->view('transaksi/daftar_pengembalian');
		$this->load->view('bottom');
	}

	public function proses_cari_kembali()
	{
		redirect("transaksi/pengembalian/" . $this->input->post("s_nisn"));
	}

	public function peminjaman($s_nisn = "")
	{

		$d['judul'] = "Transaksi Peminjaman Buku";
		
		if (!empty($s_nisn)) {
			$get = $this->db->query("SELECT * FROM sr_siswa JOIN sr_kelas ON sr_siswa.idkelas = sr_kelas.idkelas WHERE sr_siswa.s_nisn= '$s_nisn'");
			$d['combo_buku'] = $this->Combo_model->combo_buku();
			if ($get->num_rows() > 0) {
				$data_siswa = $get->row();
				$d['s_nama'] = $data_siswa->s_nama;
				$d['s_nisn'] = $data_siswa->s_nisn;
				$d['k_keterangan'] = $data_siswa->k_keterangan;
				$d['idkelas'] = $data_siswa->idkelas;
				$d['idsiswa'] = $data_siswa->idsiswa;
				$hitung_tanggungan = $this->db->query("SELECT COUNT(*) as hitung FROM peminjaman_buku_dt INNER JOIN peminjaman_buku ON peminjaman_buku_dt.id_peminjaman = peminjaman_buku.id_peminjaman WHERE id_siswa = '$data_siswa->idsiswa' AND status_input_dt = 1 AND status_pinjam_dt = 1")->row();
				if ($hitung_tanggungan->hitung > 0) {
					$d['tanggungan'] = 'Ada ' . $hitung_tanggungan->hitung . ' transkasi peminjaman yang masih berlangsung';
				} else {
					$d['tanggungan'] = 'Tidak Ada';
				}
			
				$d['peminjaman_dt'] = $this->db->query("SELECT * FROM peminjaman_buku_dt 
				INNER JOIN peminjaman_buku ON peminjaman_buku_dt.id_peminjaman = peminjaman_buku.id_peminjaman
				INNER JOIN mst_buku ON peminjaman_buku_dt.id_buku = mst_buku.id_buku WHERE peminjaman_buku.id_siswa = '$data_siswa->idsiswa' AND peminjaman_buku.status_input = 1 AND peminjaman_buku_dt.status_pinjam_dt = 1 ORDER BY peminjaman_buku_dt.id_peminjaman_dt DESC")->result_array();
			} else {
				echo '<script>
	...
	public function tambah_pinjam_buku()
	{
		...
		$jumlah = $this->db->query("SELECT jumlah_buku FROM mst_buku WHERE id_buku = '$id_buku'")->row();
		...
		if ($this->input->post("jumlah") > $stok_akhir) {
			...
			if ($this->db->insert("peminjaman_buku", $in)) {
				...
				$inDT['id_buku'] = $this->input->post("id_buku");
				...
				$inDT['status_input_dt'] = '1';
				$inDT['status_pinjam_dt'] = '1';
	...
	public function pengembalian($s_nisn = "")
	{
		...
		if (!empty($s_nisn)) {
			...
			if ($get->num_rows() > 0) {
				...
				$d['idkelas'] = $data_siswa->idkelas;
				$d['idsiswa'] = $data_siswa->idsiswa;
				$d['pengembalian_dt'] = $this->db->query("SELECT * FROM peminjaman_buku_dt 
				INNER JOIN peminjaman_buku ON peminjaman_buku_dt.id_peminjaman = peminjaman_buku.id_peminjaman
				INNER JOIN mst_buku ON peminjaman_buku_dt.id_buku = mst_buku.id_buku
         inner join sr_siswa on sr_siswa.idsiswa = peminjaman_buku.id_siswa
         WHERE sr_siswa.s_nisn = '$data_siswa->s_nisn' AND peminjaman_buku.status_input = 1 ORDER BY peminjaman_buku_dt.id_peminjaman_dt DESC")->result_array();
				$d['denda']  = $this->db->query("SELECT * FROM pengaturan_perpus WHERE id = 1")->result_array();
		     } else {
				echo '<script>
						alert("Data Siswa Tidak Ditemukan");
